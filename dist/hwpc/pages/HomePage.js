...
class HomePage extends BasePage_1.default {
    constructor(web) {
        ...
        this.HOME_LINK = ".home-link, .navbar-brand, [data-testid='home-link'], .logo, .brand";
        this.TICKETS_LINK = ".tickets-link, [href*='tickets'], [data-testid='tickets-link'], .nav-tickets, .menu-tickets";
        this.MOBILE_NAV_TOGGLE = ".navbar-toggle, .mobile-menu-toggle, [data-testid='mobile-nav-toggle'], .hamburger, .menu-toggle, .navbar-toggler";
        this.MAIN_NAVIGATION = ".main-nav, .navbar-nav, [data-testid='main-nav'], .navigation, .nav-menu";
        // Enhanced mobile-specific selectors
        this.MOBILE_MENU_OVERLAY = ".mobile-menu-overlay, .navbar-backdrop, .menu-overlay, .nav-overlay";
        this.MOBILE_MENU_CONTAINER = ".mobile-menu, .navbar-collapse, .nav-mobile, [data-mobile-menu]";
        this.MOBILE_SEARCH_TOGGLE = ".mobile-search-toggle, .search-toggle-mobile, [data-mobile-search-toggle]";
        this.MOBILE_USER_MENU = ".mobile-user-menu, .user-menu-mobile, [data-mobile-user-menu]";
        // Responsive navigation elements
        this.DESKTOP_NAVIGATION = ".desktop-nav, .nav-desktop, [data-desktop-nav]";
        this.TABLET_NAVIGATION = ".tablet-nav, .nav-tablet, [data-tablet-nav]";
        this.RESPONSIVE_MENU_ITEM = ".nav-item, .menu-item, [data-nav-item]";
        // Home page specific elements (using Constants)
        this.HERO_SECTION = Constants_1.default.HERO_SECTION;
        this.SEARCH_SECTION = Constants_1.default.SEARCH_SECTION;
        this.QUICK_ACTIONS = Constants_1.default.QUICK_ACTIONS;
        this.FEATURE_CARDS = Constants_1.default.FEATURE_CARDS;
    }
    /**
    ...
    async navigateToTicketsPage() {
        ...
        const ticketsUrl = baseUrl + Constants_1.default.TICKETS_PAGE;
     ...
     */
    async navigateToTicketsPageViaUI() {
        try {
     ...
     */
    async navigateToTicketsViaMobile() {
        try {
            // Open mobile menu first
            await this.openMobileMenu();
            // Find and click tickets link with touch-friendly interaction
            const isTicketsLinkVisible = await this.web.element(this.TICKETS_LINK, "Mobile Tickets Link").isVisible(Constants_1.default.MOBILE_WAIT_TIMEOUT);
     ...
     */
    async navigateToTicketsViaTablet() {
        try {
     ...
     */
    async navigateToTicketsViaDesktop() {
        try {
    ...
    async handleMobileNavigation() {
        ...
        if (viewport && viewport.width < Constants_1.default.RESPONSIVE_BREAKPOINT_MOBILE) {
            try {
                await this.openMobileMenu();
            }
            catch (error) {
    ...
    async openMobileMenu() {
        try {
            ...
            const isToggleVisible = await this.web.element(this.MOBILE_NAV_TOGGLE, Constants_1.default.MOBILE_MENU_TOGGLE).isVisible(Constants_1.default.MOBILE_WAIT_TIMEOUT);
            if (isToggleVisible) {
                // Ensure toggle is in viewport
                await this.scrollElementIntoView(this.MOBILE_NAV_TOGGLE);
                // Use touch-friendly click
                await this.touchFriendlyClick(this.MOBILE_NAV_TOGGLE, Constants_1.default.MOBILE_MENU_TOGGLE);
                // Wait for menu animation
                await this.web.getPage().waitForTimeout(Constants_1.default.MOBILE_MENU_SLIDE_DURATION);
                // Verify menu opened
                const isMenuVisible = await this.web.element(this.MOBILE_MENU_CONTAINER, "Mobile Menu Container").isVisible(Constants_1.default.MOBILE_WAIT_TIMEOUT);
                if (isMenuVisible) {
    ...
    async closeMobileMenu() {
        try {
            ...
            await this.web.getPage().waitForTimeout(Constants_1.default.MOBILE_MENU_FADE_DURATION);
    ...
    async verifyHomePageLoaded() {
        try {
            ...
            await this.web.element(this.HOME_LINK, Constants_1.default.HOME_LINK).waitTillVisible();
        }
        catch (error) {
    ...
    async setMobileViewport() {
        await this.web.getPage().setViewportSize(Constants_1.default.MOBILE_VIEWPORT);
    }
    /**
     * Set viewport for tablet testing
     */
    async setTabletViewport() {
        await this.web.getPage().setViewportSize(Constants_1.default.TABLET_VIEWPORT);
    }
    /**
     * Set viewport for desktop testing
     */
    async setDesktopViewport() {
        await this.web.getPage().setViewportSize(Constants_1.default.DESKTOP_VIEWPORT);
    }
    /**
     ...
     */
    async verifyMobileNavigationToggle() {
        try {
     ...
     * Verify mobile menu functionality
     */
    async verifyMobileMenuFunctionality() {
        try {
            // Open mobile menu
            await this.openMobileMenu();
     ...
     * Verify mobile-specific home page elements
     */
    async verifyMobileHomePageElements() {
        try {
     ...
     */
    async verifyMobileSearchFunctionality() {
        try {
     ...
     */
    async verifyTouchFriendlyElements() {
        try {
                ...
                for (let i = 0; i < Math.min(3, interactiveElements); i++) {
                    try {
                        ...
                        if (boundingBox) {
                            ...
                            if (minSize >= Constants_1.default.MOBILE_MIN_TOUCH_TARGET) {
                                console.log(`Touch target ${i + 1} meets minimum size requirement (${minSize}px)`);
                            }
                            else {
                                console.log(`Touch target ${i + 1} is smaller than recommended (${minSize}px < ${Constants_1.default.MOBILE_MIN_TOUCH_TARGET}px)`);
                            }
                        }
                    }
                    catch (elementError) {
     ...
     */
    async verifyCommonResponsiveElements() {
        try {
    ...
    async verifyMobileElements() {
        // Verify mobile navigation toggle is present
        try {
            const isVisible = await this.web.element(this.MOBILE_NAV_TOGGLE, Constants_1.default.MOBILE_MENU_TOGGLE).isVisible(2);
            if (isVisible) {
                console.log("Mobile navigation toggle found and verified");
            }
            else {
    ...
    async verifyTabletElements() {
        // Verify tablet-specific layout
        console.log("Verifying tablet layout");
    }
    async verifyDesktopElements() {
        // Verify desktop navigation is visible
        try {
            const isVisible = await this.web.element(this.MAIN_NAVIGATION, Constants_1.default.NAVIGATION_MENU).isVisible(2);
            if (isVisible) {
    ...
    async verifyDashboardLoaded() {
        try {
            const dashboardSelector = Constants_1.default.DASHBOARD_CONTAINER;
    ...
    async getDashboardStats() {
        try {
            const statsSelector = Constants_1.default.DASHBOARD_STATS_CARDS;
     ...
     */
    async viewRecentTickets() {
        try {
            const recentTicketsSelector = Constants_1.default.DASHBOARD_RECENT_TICKETS;
     ...
     */
    async viewActiveRoutes() {
        try {
            const activeRoutesSelector = Constants_1.default.DASHBOARD_ACTIVE_ROUTES;
    ...
    async checkNotifications() {
        try {
            const notificationsSelector = Constants_1.default.DASHBOARD_NOTIFICATIONS;
    ...
    async quickCreateTicket() {
        try {
            const quickActionSelector = `${Constants_1.default.QUICK_ACTIONS} .create-ticket-btn, .quick-create-ticket`;
    ...
    async quickSearch(searchTerm) {
        try {
            const quickSearchSelector = `${Constants_1.default.SEARCH_SECTION} input, .home-search input, .quick-search`;
            ...
            if (isQuickSearchVisible) {
                ...
                // Submit search
                const searchButtonSelector = `${Constants_1.default.SEARCH_SECTION} button, .home-search button, .quick-search-btn`;
     ...
     */
    async interactWithHomePageElement(elementType, elementIndex = 0) {
        try {
            const viewportCategory = await this.getCurrentViewportCategory();
            let selector = '';
            let elementName = '';
     ...
     */
    async verifyMobileLayoutElements() {
        try {
     ...
     */
    async verifyTabletLayoutElements() {
        try {
            const viewport = this.web.getPage().viewportSize();
     ...
     */
    async verifyDesktopLayoutElements() {
        try {
            const viewport = this.web.getPage().viewportSize();
