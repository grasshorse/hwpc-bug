...
exports.ComponentLoadError = exports.NavigationNotRenderedError = exports.SPANotInitializedError = exports.NavigationError = void 0;
const BasePage_1 = __importDefault(require("./base/BasePage"));
...
class NavigationError extends Error {
    constructor(pageName, errorType, details, screenshot) {
 ...
 */
class SPANotInitializedError extends Error {
    constructor(details, spaState, debugInfo) {
...
exports.SPANotInitializedError = SPANotInitializedError;
/**
 * NavigationNotRenderedError - Thrown when navigation container is empty
 */
class NavigationNotRenderedError extends Error {
    constructor(details, spaState, debugInfo) {
...
exports.NavigationNotRenderedError = NavigationNotRenderedError;
/**
 * ComponentLoadError - Thrown when specific components fail to load
 */
class ComponentLoadError extends Error {
    constructor(componentName, details, spaState, debugInfo) {
 ...
 */
class NavigationPage extends BasePage_1.default {
    constructor(web) {
        super(web);
    }
        ...
        try {
            ...
            spaState.componentsLoaded = await this.page.evaluate(() => {
                ...
                if (document.querySelector('[data-testid="main-navigation"]'))
                    components.push('MainNavigation');
                if (document.querySelector('[data-testid="mobile-menu-toggle"]'))
                    components.push('MobileMenuToggle');
                if (document.querySelector('.search-interface, [data-testid="search-interface"]'))
                    components.push('SearchInterface');
                // Check for page-specific components
                if (document.querySelector('[data-testid*="page"]'))
            ...
            spaState.jsErrors = await this.page.evaluate(() => {
                ...
                if (window.__jsErrors) {
                    errors.push(...window.__jsErrors);
                }
                // Check for React error boundaries
                if (window.__reactErrors) {
                    errors.push(...window.__reactErrors);
                }
                return errors;
            });
            // Get DOM ready state
        ...
        try {
            ...
            if (viewport) {
                debugInfo.viewport = {
                    ...
                    category: NavigationConstants_1.default.getViewportCategoryByWidth(viewport.width),
     ...
     */
    async captureNavigationErrorWithSPAState(pageName, errorMessage, errorType = 'navigation') {
        try {
     ...
     */
    async logJavaScriptComponentStatus() {
        try {
            ...
            const componentStatus = await this.page.evaluate(() => {
                ...
                if (window.React)
                    status.frameworks.push('React');
                if (window.Vue)
                    status.frameworks.push('Vue');
                if (window.angular)
                    status.frameworks.push('Angular');
                if (window.jQuery)
                    status.frameworks.push('jQuery');
                // Check for navigation components
                if (document.querySelector('[data-testid="main-navigation"]'))
                    status.navigationComponents.push('MainNavigation');
                if (document.querySelector('[data-testid="mobile-menu-toggle"]'))
                    status.navigationComponents.push('MobileMenuToggle');
                if (document.querySelector('.search-interface'))
                    status.navigationComponents.push('SearchInterface');
                // Check for page components
                const pageElements = document.querySelectorAll('[data-testid*="page"]');
                pageElements.forEach(el => {
                    const testId = el.getAttribute('data-testid');
                    if (testId)
                        status.pageComponents.push(testId);
                });
     ...
     */
    async waitForSPAInitialization() {
        const startTime = Date.now();
        try {
            ...
            await this.page.waitForFunction(() => {
                ...
                return window.app !== undefined ||
                    document.readyState === 'complete' ||
                    document.querySelector('script[src*="app"]') !== null;
            }, { timeout: NavigationConstants_1.default.SPA_TIMEOUTS.initialization });
            ...
            throw new SPANotInitializedError(`SPA initialization timeout: ${error.message}`, spaState, debugInfo);
        }
    }
    /**
     ...
     */
    async waitForNavigationRendered() {
        const startTime = Date.now();
        const debugInfo = await this.captureSPADebugInfo();
        ...
        try {
            ...
            const navigationSelectors = NavigationConstants_1.default.NAVIGATION_CONTAINER.split(', ');
            ...
            const mainNavSelectors = NavigationConstants_1.default.MAIN_NAVIGATION.split(', ');
            ...
            }, { timeout: NavigationConstants_1.default.SPA_TIMEOUTS.navigationRender });
            ...
            throw new NavigationNotRenderedError(`Navigation rendering timeout: ${error.message}`, spaState, debugInfo);
        }
    }
    /**
    ...
    async verifyNavigationStructure() {
        try {
            ...
            for (const selector of navigationSelectors) {
                debugInfo.selectors.attempted.push(selector.trim());
                const containerExists = await this.web.element(selector.trim(), "Navigation Container").isVisible(1);
                if (containerExists) {
            ...
            validationResults.mainNavigation = await this.web.element(NavigationConstants_1.default.MAIN_NAVIGATION, "Main Navigation").isVisible(2);
            ...
            if (this.isMobile) {
                validationResults.mobileMenuToggle = await this.web.element(NavigationConstants_1.default.MOBILE_MENU_TOGGLE, "Mobile Menu Toggle").isVisible(2);
            }
            else {
            ...
            if (allValid) {
                ...
                const spaState = await this.captureSPAState();
                await this.captureNavigationErrorWithSPAState('navigation-structure', 'Navigation structure verification failed', 'component');
            }
            return allValid;
        }
        catch (error) {
            ...
            const spaState = await this.captureSPAState();
            const debugInfo = await this.captureSPADebugInfo();
            await this.captureNavigationErrorWithSPAState('navigation-structure', error.message, 'component');
            return false;
        }
    }
     ...
     */
    async waitForSPARouteChange(pageName) {
        try {
            console.log(`Waiting for SPA route change to ${pageName}...`);
            const pageConfig = NavigationConstants_1.default.getPageConfig(pageName);
            if (!pageConfig) {
                throw new Error(`Page configuration not found for: ${pageName}`);
            }
            // Wait for URL to match the expected route patterns
            await this.page.waitForFunction((expectedPatterns) => {
                const currentUrl = window.location.href;
                return expectedPatterns.some((pattern) => currentUrl.includes(pattern) ||
                    currentUrl.endsWith(pattern) ||
                    new RegExp(pattern.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')).test(currentUrl));
            }, [pageConfig.url, ...pageConfig.urlPatterns], { timeout: NavigationConstants_1.default.SPA_TIMEOUTS.routeChange });
     ...
     */
    async waitForJavaScriptRenderedContent(pageName) {
        try {
            console.log(`Waiting for JavaScript-rendered content for ${pageName}...`);
            // Wait for DOM to be interactive
            await this.page.waitForLoadState('domcontentloaded', {
                timeout: NavigationConstants_1.default.SPA_TIMEOUTS.componentMount
            });
            ...
            await this.page.waitForFunction(() => {
                const loadingIndicators = document.querySelectorAll('.loading, .spinner, .loader, [data-loading="true"], .loading-spinner');
     ...
     */
    verifySPARouteUrl(currentUrl, pageName) {
        const pageConfig = NavigationConstants_1.default.getPageConfig(pageName);
        if (!pageConfig)
            return false;
        ...
        for (const hashRoute of hashRoutes) {
            if (currentUrl.includes(hashRoute))
                return true;
        }
        return false;
    }
     ...
     */
    async waitForSPAPageTitle(pageName) {
        try {
            ...
            await this.page.waitForFunction((expectedPatterns) => {
                ...
                return expectedPatterns.some((pattern) => currentTitle.includes(pattern) ||
                    new RegExp(pattern, 'i').test(currentTitle));
            }, [pageConfig.title, ...pageConfig.titlePatterns], { timeout: NavigationConstants_1.default.SPA_TIMEOUTS.routeChange });
     ...
     */
    async waitForJavaScriptElement(selector, elementName, timeout = NavigationConstants_1.default.SPA_TIMEOUTS.componentMount) {
        try {
    ...
    async navigateToPage(pageName) {
        try {
            ...
            const navigationSelector = NavigationConstants_1.default.getNavigationSelector(pageName, this.isMobile);
            ...
            const timeout = NavigationConstants_1.default.getTimeout(viewportCategory, 'pageLoad');
     ...
     */
    async verifyPageLoaded(pageName) {
        const startTime = Date.now();
        const validation = {
            url: '',
            title: '',
        ...
        try {
            ...
            if (!NavigationConstants_1.default.matchesPageTitle(currentTitle, pageName)) {
            ...
            const pageIdentifierSelector = NavigationConstants_1.default.getPageIdentifierSelector(pageName, this.isMobile);
            const isPageIdentifierVisible = await this.waitForJavaScriptElement(pageIdentifierSelector, `${pageName} Page Identifier`, NavigationConstants_1.default.SPA_TIMEOUTS.componentMount);
            ...
            const requiredElements = NavigationConstants_1.default.getRequiredElements(pageName);
            for (const elementSelector of requiredElements) {
                const isElementVisible = await this.waitForJavaScriptElement(elementSelector, `Required Element`, NavigationConstants_1.default.SPA_TIMEOUTS.componentMount);
                if (!isElementVisible) {
                    validation.errors.push(`Required element not found: ${elementSelector}`);
                }
            }
            // Verify search interface with dynamic content wait
            const searchInterfaceSelector = NavigationConstants_1.default.getSearchInterfaceSelector(pageName, this.isMobile);
            validation.searchInterfacePresent = await this.waitForJavaScriptElement(searchInterfaceSelector, `${pageName} Search Interface`, NavigationConstants_1.default.SPA_TIMEOUTS.componentMount);
     ...
     */
    async verifyResponsiveInterface() {
        try {
            ...
            if (isResponsive) {
                ...
                await this.captureNavigationErrorWithSPAState('responsive-interface', `Responsive verification failed: ${errors.join(', ')}`, 'component');
            }
            return isResponsive;
        }
        catch (error) {
            ...
            await this.captureNavigationErrorWithSPAState('responsive-interface', error.message, 'component');
            return false;
        }
    }
     ...
     */
    async waitForResponsiveElementsToRender(viewportCategory) {
        try {
            console.log(`Waiting for responsive elements to render for ${viewportCategory}...`);
            // Wait for responsive containers to be populated
            await this.page.waitForFunction((viewport) => {
                // Check for responsive containers
                const containers = document.querySelectorAll('.container, .container-fluid, .responsive-container, [data-responsive]');
                if (containers.length === 0)
                    return false;
                // Check for viewport-specific elements
                if (viewport === 'mobile') {
                    const mobileElements = document.querySelectorAll('.mobile-container, [data-mobile], .d-block-mobile, .mobile-nav');
                    return mobileElements.length > 0;
                }
                else if (viewport === 'tablet') {
                    const tabletElements = document.querySelectorAll('.tablet-container, [data-tablet], .d-block-tablet');
                    return tabletElements.length > 0 || containers.length > 0;
                }
                else {
                    const desktopElements = document.querySelectorAll('.desktop-container, [data-desktop], .d-block-desktop');
                    return desktopElements.length > 0 || containers.length > 0;
                }
            }, viewportCategory, { timeout: NavigationConstants_1.default.SPA_TIMEOUTS.componentMount });
     ...
     */
    async verifyTouchTargetSizesWithDynamicContent() {
        try {
     ...
     */
    async verifyResponsiveNavigationWithSPA() {
        try {
            ...
            if (viewportCategory === 'mobile') {
                ...
                const mobileNavVisible = await this.waitForJavaScriptElement(NavigationConstants_1.default.MOBILE_MENU_TOGGLE, "Mobile Menu Toggle", NavigationConstants_1.default.SPA_TIMEOUTS.componentMount);
                if (!mobileNavVisible) {
                ...
                const mainNavVisible = await this.waitForJavaScriptElement(NavigationConstants_1.default.MAIN_NAVIGATION, "Main Navigation", NavigationConstants_1.default.SPA_TIMEOUTS.componentMount);
                if (!mainNavVisible) {
     ...
     */
    async verifyResponsiveSearchInterfaceWithDynamicContent() {
        try {
            ...
            const searchSelectors = viewportCategory === 'mobile' ?
                NavigationConstants_1.default.MOBILE_SEARCH_CONTAINER :
                NavigationConstants_1.default.SEARCH_CONTAINER;
            const searchInterfaceVisible = await this.waitForJavaScriptElement(searchSelectors, "Search Interface", NavigationConstants_1.default.SPA_TIMEOUTS.componentMount);
            ...
            const searchInputSelectors = viewportCategory === 'mobile' ?
                NavigationConstants_1.default.MOBILE_SEARCH_INPUT :
                NavigationConstants_1.default.SEARCH_INPUT;
            ...
            const searchResponsive = await this.page.evaluate((viewport) => {
                const searchContainers = document.querySelectorAll('.search-container, .search-interface, [data-search]');
     ...
     */
    async handleMobileNavigation(navigationSelector, pageName) {
        try {
            ...
            if (!isMobileMenuVisible) {
                ...
                if (isToggleVisible) {
                    ...
                    await this.page.waitForTimeout(NavigationConstants_1.default.MOBILE_MENU_CONFIG.slideInDuration);
     ...
     * Updated to use correct selectors and wait for dynamically rendered elements
     */
    async handleDesktopNavigation(navigationSelector, pageName) {
        try {
    ...
    async getNavigationLinks() {
        try {
            ...
            const pageNames = NavigationConstants_1.default.getPageNames();
            ...
            if (this.isMobile) {
                ...
                if (isMobileMenuVisible) {
                    ...
                    if (isToggleVisible) {
                        await this.touchFriendlyClick(mobileToggleSelector, "Mobile Menu Toggle");
                        await this.page.waitForTimeout(NavigationConstants_1.default.MOBILE_MENU_CONFIG.slideOutDuration);
                    }
                }
            }
     ...
     */
    async captureNavigationError(pageName, errorDetails) {
        try {
            const viewportCategory = await this.getCurrentViewportCategory();
     ...
     */
    async retryNavigation(pageName, maxRetries = 3) {
        let lastError = null;
     ...
     */
    async fallbackNavigation(pageName) {
        try {
            ...
            const baseUrl = process.env.BASE_URL || Constants_1.default.DEFAULT_BASE_URL;
     ...
     */
    async verifyMobileResponsiveElementsInternal() {
        try {
            console.log("Verifying mobile responsive elements...");
            // Check for mobile menu toggle
            const isMobileToggleVisible = await this.web.element(NavigationConstants_1.default.MOBILE_MENU_TOGGLE, "Mobile Menu Toggle").isVisible(2);
            if (!isMobileToggleVisible) {
                console.log("Mobile menu toggle not found");
                return false;
            }
            // Check for mobile-specific containers
            const isMobileContainerVisible = await this.web.element(NavigationConstants_1.default.MOBILE_CONTAINER, "Mobile Container").isVisible(2);
            // Check for mobile-hidden elements are actually hidden
            const mobileHiddenElements = await this.page.locator(NavigationConstants_1.default.MOBILE_HIDDEN).count();
            if (mobileHiddenElements > 0) {
                const visibleHiddenElements = await this.page.locator(`${NavigationConstants_1.default.MOBILE_HIDDEN}:visible`).count();
     ...
     */
    async verifyTabletResponsiveElementsInternal() {
        try {
            console.log("Verifying tablet responsive elements...");
            // Check for tablet-specific containers
            const isTabletContainerVisible = await this.web.element(NavigationConstants_1.default.TABLET_CONTAINER, "Tablet Container").isVisible(2);
            // Verify tablet-specific visibility classes
            const tabletVisibleElements = await this.page.locator(NavigationConstants_1.default.TABLET_VISIBLE).count();
            if (tabletVisibleElements > 0) {
                const hiddenTabletElements = await this.page.locator(`${NavigationConstants_1.default.TABLET_VISIBLE}:hidden`).count();
     ...
     */
    async verifyDesktopResponsiveElementsInternal() {
        try {
            console.log("Verifying desktop responsive elements...");
            // Check for desktop-specific containers
            const isDesktopContainerVisible = await this.web.element(NavigationConstants_1.default.DESKTOP_CONTAINER, "Desktop Container").isVisible(2);
            // Verify desktop-specific visibility classes
            const desktopVisibleElements = await this.page.locator(NavigationConstants_1.default.DESKTOP_VISIBLE).count();
            if (desktopVisibleElements > 0) {
                const hiddenDesktopElements = await this.page.locator(`${NavigationConstants_1.default.DESKTOP_VISIBLE}:hidden`).count();
    ...
    async verifyTouchTargetSizes() {
        try {
            ...
            for (const target of touchTargets) { // Check targets
                ...
                if (boundingBox) {
                    const isAdequate = NavigationConstants_1.default.isTouchTargetAdequate(boundingBox.width, boundingBox.height);
                    if (isAdequate) {
                        validTargets++;
                    }
                }
    ...
    async verifyMobileNavigation() {
        try {
            ...
            // Test mobile menu functionality
            await this.touchFriendlyClick(NavigationConstants_1.default.MOBILE_MENU_TOGGLE, "Mobile Menu Toggle");
            await this.page.waitForTimeout(NavigationConstants_1.default.MOBILE_MENU_CONFIG.slideInDuration);
            const isMobileMenuVisible = await this.web.element(NavigationConstants_1.default.MOBILE_MENU_CONTAINER, "Mobile Menu").isVisible(2);
            // Close menu
            if (isMobileMenuVisible) {
                await this.touchFriendlyClick(NavigationConstants_1.default.MOBILE_MENU_TOGGLE, "Mobile Menu Toggle");
                await this.page.waitForTimeout(NavigationConstants_1.default.MOBILE_MENU_CONFIG.slideOutDuration);
     ...
     */
    async verifyTabletNavigation() {
        try {
            // Check for main navigation visibility
            const isMainNavVisible = await this.web.element(NavigationConstants_1.default.MAIN_NAVIGATION, "Main Navigation").isVisible(2);
    ...
    async verifyDesktopNavigation() {
        try {
            ...
            const isNavMenuVisible = await this.web.element(NavigationConstants_1.default.NAVIGATION_MENU, "Navigation Menu").isVisible(2);
     ...
     */
    async verifyResponsiveSearchInterface() {
        try {
     ...
     */
    async initializeSPAAfterNavigation() {
        try {
    ...
    async validatePageElements() {
        try {
            ...
            const isResponsiveContainerVisible = await this.web.element(NavigationConstants_1.default.RESPONSIVE_CONTAINER, "Responsive Container").isVisible(2);
     ...
     */
    async attemptErrorRecovery(pageName) {
        try {
     ...
     */
    async navigateWithViewportDetection(pageName) {
        try {
