 ...
 */
class CommonPage extends BasePage_1.default {
    constructor(web) {
        super(web);
        // Updated selectors for HWPC mobile-first UI - using more generic selectors
        this.SUCCESS_MESSAGE_TEXT = "h1, .page-title, .alert-success, .success-message, .page-header h1";
        this.SEARCH_TEXTBOX = "input[type='search'], input[name='search'], .search-input, #search-input, [data-testid='search-input'], input[placeholder*='search' i]";
        this.SEARCH_BUTTON = "button[type='submit'], .search-button, .btn-search, [data-testid='search-button'], input[type='submit'], button:has-text('Search')";
        this.LOADING_INDICATOR = ".loading, .spinner, .loading-indicator, [data-testid='loading']";
        // Enhanced mobile navigation selectors
        this.MOBILE_MENU_TOGGLE = ".navbar-toggle, .mobile-menu-toggle, .hamburger-menu, [data-testid='mobile-menu-toggle'], .navbar-toggler, .hamburger, .mobile-menu-btn";
        this.MY_ACCOUNT_LINK = "//li[contains(@class,'dropdown')]//span[contains(text(),'My account')], .account-link, [data-testid='account-link']";
        this.MENU_LINK = "//ul[contains(@class,'dropdown-menu')]//span[contains(text(),'{0}')], .menu-item[data-menu='{0}']";
        // Enhanced mobile-specific navigation selectors
        this.MOBILE_NAV_MENU = ".mobile-nav, .navbar-collapse, [data-testid='mobile-nav'], .mobile-menu, .nav-mobile";
        this.MOBILE_MENU_OVERLAY = ".mobile-menu-overlay, .navbar-backdrop, .menu-overlay";
        this.MOBILE_SEARCH_CONTAINER = ".mobile-search, .search-mobile, [data-mobile-search]";
    }
    /**
     * Search for tickets with mobile-first approach
     * @param searchTerm - The search term to look for
     */
    async searchTicket(searchTerm) {
        try {
            ...
            const searchInput = this.web.editBox(this.SEARCH_TEXTBOX, Constants_1.default.SEARCH_INPUT);
            ...
            const searchButton = this.web.element(this.SEARCH_BUTTON, Constants_1.default.SEARCH_BUTTON);
    ...
    async waitForLoadingComplete() {
        try {
            ...
            await this.web.element(this.LOADING_INDICATOR, Constants_1.default.LOADING_INDICATOR).waitTillInvisible();
        }
        catch (error) {
     ...
     */
    async handleMobileMenuToggle() {
        try {
            // Use touch-friendly click with proper timing
            const isToggleVisible = await this.web.element(this.MOBILE_MENU_TOGGLE, Constants_1.default.MOBILE_MENU_TOGGLE).isVisible(Constants_1.default.MOBILE_WAIT_TIMEOUT);
            if (isToggleVisible) {
                ...
                await this.touchFriendlyClick(this.MOBILE_MENU_TOGGLE, Constants_1.default.MOBILE_MENU_TOGGLE);
                // Wait for menu animation to complete
                await this.web.getPage().waitForTimeout(Constants_1.default.MOBILE_MENU_SLIDE_DURATION);
                // Verify menu opened
                const isMenuVisible = await this.web.element(this.MOBILE_NAV_MENU, Constants_1.default.NAVIGATION_MENU).isVisible(Constants_1.default.MOBILE_WAIT_TIMEOUT);
                if (isMenuVisible) {
     ...
     */
    async toggleMobileDropdown(dropdownSelector, dropdownName) {
        try {
            const viewport = this.web.getPage().viewportSize();
            if (viewport && viewport.width < Constants_1.default.RESPONSIVE_BREAKPOINT_MOBILE) {
    ...
    async closeMobileMenu() {
        try {
            ...
            if (viewport && viewport.width < Constants_1.default.RESPONSIVE_BREAKPOINT_MOBILE) {
                ...
                const isMenuVisible = await this.web.element(this.MOBILE_NAV_MENU, Constants_1.default.NAVIGATION_MENU).isVisible(1);
                if (isMenuVisible) {
    ...
    async logout() {
        ...
        await this.web.element(this.MY_ACCOUNT_LINK, Constants_1.default.MY_ACCOUNT).hover();
        await this.web.element(StringUtil_1.default.formatString(this.MENU_LINK, Constants_1.default.LOGOUT), Constants_1.default.LOGOUT).click();
    }
    async navigateToRegisterUser() {
        await this.toggleMobileMenu();
        await this.web.element(this.MY_ACCOUNT_LINK, Constants_1.default.MY_ACCOUNT).hover();
        await this.web.element(StringUtil_1.default.formatString(this.MENU_LINK, Constants_1.default.REGISTER), Constants_1.default.REGISTER).click();
    }
    /**
     * Verify the message displayed on title of the page with mobile considerations
     * @param message
     */
    async verifyTitleMessage(message) {
        // Wait for message to be visible (important for mobile loading)
        await this.web.element(this.SUCCESS_MESSAGE_TEXT, Constants_1.default.MESSAGE).waitTillVisible();
        const actualMsg = await this.web.element(this.SUCCESS_MESSAGE_TEXT, Constants_1.default.MESSAGE).getTextContent();
        await Assert_1.default.assertEquals(actualMsg, message, Constants_1.default.MESSAGE);
    }
    /**
    ...
    async swipe(startSelector, direction, distance = Constants_1.default.SWIPE_DISTANCE) {
        try {
            ...
            if (boundingBox) {
                ...
                await this.web.getPage().mouse.up();
                await this.web.getPage().waitForTimeout(Constants_1.default.SWIPE_DURATION);
            }
        }
        catch (error) {
    ...
    async scrollElementIntoView(selector) {
        try {
            ...
            await this.web.getPage().waitForTimeout(Constants_1.default.MOBILE_SCROLL_DELAY);
            ...
            if (!isInViewport) {
                ...
                await this.web.getPage().waitForTimeout(Constants_1.default.MOBILE_SCROLL_TIMEOUT);
            }
        }
        catch (error) {
    ...
    async viewportAwareClick(selector, elementName) {
        try {
            ...
            else if (viewport && viewport.width < Constants_1.default.RESPONSIVE_BREAKPOINT_TABLET) {
                // Tablet interaction
    ...
    async viewportAwareHover(selector, elementName) {
        try {
            const viewport = this.web.getPage().viewportSize();
            if (viewport && viewport.width < Constants_1.default.RESPONSIVE_BREAKPOINT_TABLET) {
     ...
     */
    async mobileOptimizedSearch(searchTerm) {
        try {
            const viewport = this.web.getPage().viewportSize();
            if (viewport && viewport.width < Constants_1.default.RESPONSIVE_BREAKPOINT_MOBILE) {
                ...
                if (isMobileSearchVisible) {
                    ...
                    await this.web.getPage().waitForTimeout(Constants_1.default.MOBILE_KEYBOARD_SHOW_DELAY);
                }
            }
