...
class TestData {
     ...
     */
    static generateEmail(name, domain = 'example.com') {
     ...
     */
    static generatePerformanceTestData(recordCount) {
        const tickets = [];
        const customers = [];
        const routes = [];
        for (let i = 0; i < recordCount; i++) {
            tickets.push({
                ...
                createdAt: this.generateRandomDate(new Date(Date.now() - 30 * 24 * 60 * 60 * 1000), // 30 days ago
                new Date())
            });
            if (i < recordCount / 2) { // Generate fewer customers than tickets
                customers.push({
                    ...
                    createdAt: this.generateRandomDate(new Date(Date.now() - 365 * 24 * 60 * 60 * 1000), // 1 year ago
                    new Date())
                });
            }
            if (i < recordCount / 10) { // Generate fewer routes
                routes.push({
                    ...
                    date: this.generateRandomDate(new Date(), new Date(Date.now() + 7 * 24 * 60 * 60 * 1000) // Next 7 days
                    )
                });
            }
        }
    ...
    static generateErrorScenarioData() {
        return {
            ...
            MALFORMED_CUSTOMER: {
                ...
                phone: "123",
                serviceTypes: [],
                accountStatus: 'deleted' // Invalid status
            },
            CONFLICTING_ROUTE: {
                id: "CONFLICT",
    ...
    static generateComprehensiveTestDataSet(options = {}) {
        const { ticketCount = 10, customerCount = 5, routeCount = 3, technicianCount = 3, includeRelationships = true, includeHistoricalData = false } = options;
        ...
        const tickets = Array.from({ length: ticketCount }, (_, i) => ({
            ...
            scheduledDate: this.generateRandomDate(new Date(), new Date(Date.now() + 30 * 24 * 60 * 60 * 1000) // Next 30 days
            ),
            createdAt: this.generateRandomDate(new Date(Date.now() - 7 * 24 * 60 * 60 * 1000), // Last 7 days
            new Date())
        }));
        ...
        if (includeHistoricalData) {
            serviceHistory = Array.from({ length: customerCount * 3 }, (_, i) => ({
                id: `HIST-${Date.now()}-${i}`,
                date: this.generateRandomDate(new Date(Date.now() - 365 * 24 * 60 * 60 * 1000), // Last year
                new Date()),
     ...
     */
    static generateBusinessScenarioData(scenario) {
     ...
     */
    static generateLoadTestData(requestCount) {
        return Array.from({ length: requestCount }, (_, i) => ({
            ...
            expectedResponseTime: Math.floor(Math.random() * 1000) + 200,
...
// ===== PRIORITY LEVELS =====
TestData.PRIORITY_LEVELS = ['low', 'medium', 'high', 'urgent'];
// ===== STATUS OPTIONS =====
TestData.TICKET_STATUSES = ['open', 'assigned', 'in_progress', 'completed', 'cancelled', 'scheduled'];
TestData.CUSTOMER_STATUSES = ['active', 'inactive', 'suspended'];
TestData.ROUTE_STATUSES = ['draft', 'active', 'completed', 'cancelled'];
